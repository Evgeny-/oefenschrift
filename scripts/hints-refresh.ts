...
if (!reason) { console.error('Give a reason, e.g. "batch 004 integrated (closed items only)".'); process.exit(2); }
...
if (digest(readFileSync('content/hints/sentence-starters.json')) !== review.source_sha256) { console.error('The starters file changed; a focused review is needed, not a refresh.'); process.exit(1); }
const open = catalogue.filter((i: any) => !i.questions?.length);
const missing = open.filter((i: any) => !Array.isArray(starters[i.id]) || starters[i.id].length !== i.criteria.length);
const extra = Object.keys(starters).filter(id => !open.some((i: any) => i.id === id));
if (missing.length || extra.length) { console.error(`Open tasks changed (missing: ${missing.map((i: any) => i.id).join(', ') || 'none'}; extra: ${extra.join(', ') || 'none'}); a focused review is needed.`); process.exit(1); }
const previous = review.catalogue_sha256, current = digest(catalogueBytes);
if (previous === current) { console.log('Catalogue hash unchanged; nothing to do.'); process.exit(0); }
review.catalogue_sha256 = current;
review.checks = {...review.checks, open_catalogue_items: open.length, covered_item_ids: open.length};
review.hash_refreshes = [...(review.hash_refreshes || []), {date: new Date().toISOString().slice(0, 10), reason, from: previous, to: current, open_items_unchanged: true}];
