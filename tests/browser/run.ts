...
const port = await freePort(), bidiPort = await freePort();
const origin = `http://127.0.0.1:${port}`;
const production = process.argv.includes('--production');
const environment = {...process.env, PORT: String(port), TEST_ORIGIN: origin,
  NODE_ENV: production ? 'production' : 'development',
  FIREFOX_BIDI_PORT: String(bidiPort), INBURGERING_DB: resolve(directory, 'test.sqlite3'),
  INBURGERING_ADMIN_SECRET: randomBytes(32).toString('hex'),
  INBURGERING_ADMIN_USER: 'browser-test', INBURGERING_ADMIN_PASSWORD: 'browser-test-password', INBURGERING_OFFLINE: '1',
  OPENAI_API_KEY: 'browser-test-no-provider', ELEVENLABS_API_KEY: 'browser-test-no-provider'};
...
const server = spawn(process.execPath, ['node_modules/tsx/dist/cli.mjs', 'server/index.ts'], {
  env: environment, detached: true, stdio: ['ignore', serverLog.fd, serverLog.fd],
});
const binary = process.env.FIREFOX_BINARY || (process.platform === 'darwin'
  ? '/Applications/Firefox.app/Contents/MacOS/firefox' : 'firefox');
await mkdir(resolve(directory, 'profile'));
const firefox = spawn(binary, ['--headless', '--no-remote', '--profile', resolve(directory, 'profile'),
  '--remote-debugging-port', String(bidiPort), 'about:blank'], {
  detached: true, stdio: ['ignore', browserLog.fd, browserLog.fd],
});
let launchError: Error | undefined;
for (const child of [server, firefox]) child.on('error', error => { launchError = error; });
try {
  await waitUntil(async () => {
    if (launchError) throw launchError;
    try { return (await fetch(origin + '/api/status')).ok; } catch { return false; }
  }, 'Application');
  await waitUntil(() => new Promise(done => {
  ...
  for (const test of production ? ['production'] : ['infrastructure', 'refinement']) {
    console.log(`\nFirefox: ${test}`);
    const child = spawn(process.execPath, [`tests/browser/${test}.mjs`], {env: environment, stdio: 'inherit'});
  ...
  await serverLog.close(); await browserLog.close();
}
