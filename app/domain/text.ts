...
export function evidenceFor(item,q){
  ...
  if(quotes.length)return quotes.filter(x=>typeof x==='string'&&x&&item.text.includes(x));
  return (q.evidence_paragraphs||[]).map(n=>item.text.split('\n\n')[n-1]).filter(Boolean);
}
export function highlightParts(text,quotes=[]){
  const ranges=[];
  for(const quote of quotes){if(!quote)continue;const start=text.indexOf(quote);if(start>=0)ranges.push([start,start+quote.length]);}
  ...
  const out=[];let i=0;
  for(const [a,b] of merged){if(a>i)out.push({text:text.slice(i,a),marked:false});out.push({text:text.slice(a,b),marked:true});i=b;}
  if(i<text.length)out.push({text:text.slice(i),marked:false});return out;
}
export function wordDiff(before,after){
  ...
  if(a.length*b.length>1500000)return [{type:'remove',text:before},{type:'add',text:after}];
  ...
  const out=[];function add(type,text){if(out.at(-1)?.type===type)out.at(-1).text+=text;else out.push({type,text});}
  let i=0,j=0;
  while(i<a.length||j<b.length){if(i<a.length&&j<b.length&&a[i]===b[j]){add('same',a[i++]);j++;}else if(j<b.length&&(i===a.length||rows[i][j+1]>rows[i+1][j]))add('add',b[j++]);else add('remove',a[i++]);}
  return out;
}
