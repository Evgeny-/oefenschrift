...
export function pageSeo(
  ...
  const title = item
    ? `${item.title} · ${item.level} ${subjectNames[item.part]}`
    : set
      ? `${set.level} ${subjectNames[set.part]} · Oefenset ${set.number}`
      : subject
        ? `${level} ${subjectNames[route]} oefenen`
        : subjectNames[route] || 'Oefenen';
  let description =
  ...
  if (item)
    description = `${item.level} ${subjectNames[item.part]}: ${item.prompt || item.text || item.transcript || item.title}`;
  else if (set)
    description = `Oefenset ${set.number} voor ${set.level} ${subjectNames[set.part].toLowerCase()}: ${set.ids.length} opdrachten om zelfstandig te oefenen, met uitleg of oefenfeedback.`;
  else if (subject)
    description = available.length
      ? `Oefen ${subjectNames[route].toLowerCase()} op niveau ${level} met ${available.length} eigen opdrachten. Werk in korte oefensets en bekijk de uitleg of oefenfeedback.`
      : `Oefeningen voor ${level} ${subjectNames[route].toLowerCase()} zijn nog niet beschikbaar.`;
  else if (route === 'knm')
    description =
  ...
  const noindex =
    ['progress', 'session', 'privacy', 'mock'].includes(route) || (subject && !available.length);
  return {
    title: title + ' | Inburgering',
    description: description.replace(/\s+/g, ' ').slice(0, 160),
    canonical: new URL(routePath(route, level), origin).href,
    noindex,
  };
}
export function sitemapPaths(catalogue: Exercise[], sets: PracticeSet[]) {
  ...
  for (const item of catalogue) {
    paths.add(routePath(item.part, item.level === 'KNM' ? 'A2' : item.level));
    paths.add(routePath('exercise/' + item.id));
  }
