...
export function AudioPlayback({
  ...
  const duration =
    mediaDuration || (Number.isFinite(knownDuration) && knownDuration > 0 ? knownDuration : 0);
    ...
    <div className="audio-player" role="group" aria-label={label}>
      ...
      <div className="audio-main">
        <button
          ...
          onClick={play}
          disabled={suspended}
          aria-label={playing ? t('Pauzeren', 'Pause') : t('Afspelen', 'Play')}
        >
          <PlayIcon playing={playing} />
        </button>
        <div className={`waveform ${peaks.length ? '' : 'waveform-timeline'}`}>
          <div className="waveform-bars" aria-hidden="true">
            {peaks.map((peak, i) => (
              <i
                key={i}
                className={duration && i / peaks.length < time / duration ? 'heard' : ''}
          ...
          <input
            ...
            disabled={!duration || suspended}
            onChange={(e) => seek(Number(e.target.value))}
            aria-label={t('Afspeelpositie', 'Playback position')}
            aria-valuetext={`${formatTime(time)} / ${duration ? formatTime(duration) : '—:—'}`}
          />
        </div>
      </div>
      <div className="audio-meta">
        ...
        <Segments
          ...
          compact
          hideLabel
          label={t('Snelheid', 'Speed')}
          value={rate}
          options={[0.75, 1, 1.25].map((value) => ({ value, label: `${value}×` }))}
          onChange={(value) => {
            setRate(value);
      ...
      {error && (
        <p className="feedback-error" role="alert">
          {t('Afspelen lukt niet. Probeer het opnieuw.', 'Playback failed. Try again.')}
        </p>
      )}
    </div>
  );
