...
export default function ThemeSwitch({initial}:{initial:'light'|'dark'|'system'}){
  ...
  useEffect(()=>{
    ...
    const apply=()=>{document.documentElement.dataset.theme=resolveTheme(theme,system.matches);};
  ...
  const choose=value=>{
    ...
    try{const key='inburgering.study.v2',saved=JSON.parse(localStorage.getItem(key)||'null');if(saved?.settings){saved.settings.theme=value;localStorage.setItem(key,JSON.stringify(saved));}}catch{}
  };
  return <Segments id="ops-theme" label="Theme" value={theme} options={[{value:'light',label:<ThemeIcon theme="light"/>,ariaLabel:'Light'},{value:'dark',label:<ThemeIcon theme="dark"/>,ariaLabel:'Dark'},{value:'system',label:<ThemeIcon theme="system"/>,ariaLabel:'System'}]} onChange={choose}/>;
}
