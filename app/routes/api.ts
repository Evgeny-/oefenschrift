...
const calls: number[] = [],
  eventCalls: number[] = [];
function limited(list: number[], max: number, now: number) {
...
export function loader({ request, params }) {
  ...
  if (params.service === 'status')
    return Response.json(
      {
        feedback: !!keys().OPENAI_API_KEY,
        speech: !!keys().ELEVENLABS_API_KEY,
        reports: true,
...
export async function action({ request, params }) {
  ...
  if (service === 'events') {
    if (limited(eventCalls, 120, now))
      return Response.json({ error: 'Too many events.' }, { status: 429 });
    const visitor = visitorHash(data?.visitor);
    if (!visitor) throw new Response('Invalid visitor', { status: 400 });
    try {
      return Response.json({ recorded: store.recordEvents(visitor, data.events, now) });
    } catch (error) {
      return Response.json(
  ...
  if (limited(calls, 30, now))
    return Response.json({ error: 'Please wait a minute before trying again.' }, { status: 429 });
  let failed = false;
  try {
    if (service === 'reports') return Response.json(store.report(data));
    const item = store
      .catalogue()
      .find((i) => i.id === data.id && ['writing', 'speaking'].includes(i.part));
    if (!item) throw new StoreError('Invalid exercise.');
    store.serviceEvent(service, visitorHash(data.visitor), item.id, now);
    if (service === 'feedback') {
      if (
        typeof data.answer !== 'string' ||
        !data.answer.trim() ||
        data.answer.length > 3000 ||
        !['nl', 'en'].includes(data.lang)
      )
        throw new StoreError('Invalid answer.');
      return Response.json(
        await sharedFeedback(item, data.answer, data.lang, (usage) =>
    ...
    return Response.json(
      {
        error:
          error instanceof StoreError || error instanceof ServiceError
            ? error.message
            : 'Service unavailable. Please retry.',
      },
      { status: error instanceof StoreError || error instanceof ServiceError ? error.status : 502 },
    );
  } finally {
