...
export async function loader({ request }) {
  ...
  if (!cache || Date.now() - cache.at > 300000)
    cache = {
      at: Date.now(),
      value: { eleven: await elevenLabsSubscription(), openai: await openAiCosts() },
    };
...
export default function Services() {
  const { eleven, openai, daily, services, stats, tokens, model, openaiKey, adminKey } =
  ...
  const costChart = useMemo(
    () => (t) => ({
      ...axes(t),
      xAxis: { ...axes(t).xAxis, data: (openai?.days || []).map((d) => shortDay(d.day)) },
      yAxis: {
        ...axes(t).yAxis,
      ...
      series: [
        bar(
          'Cost',
          (openai?.days || []).map((d) => Number(d.cost.toFixed(2))),
          t.series[0],
          t,
        ),
      ],
            ...
            <>
              <div className="tiles-inline">
                <Tile
                  ...
                  detail={`of ${number(credits.limit)} · ${percent(credits.limit - credits.used, credits.limit)} of the ${credits.tier || 'current'} plan`}
              ...
              <p className="small">
                ...
                {number(
                  stats
                    .filter(
                      (row) =>
                        row.service === 'transcribe' &&
                        row.day >= new Date().toISOString().slice(0, 8),
                    )
              ...
              {openai?.configured && !openai.error ? (
                <>
                  <p className="small">
                    ...
                    <strong>${openai.total.toFixed(2)}</strong>
                  </p>
